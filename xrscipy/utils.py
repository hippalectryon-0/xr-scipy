...
def wrap_dataset(
    func: Callable, y: xr.Dataset | xr.DataArray, *dims, **kwargs
) -> xr.Dataset:
    """
    ...
    for d in dims:
        if d not in y.dims:
            raise ValueError(
                f"{d} is not a valid dimension for the object. The valid dimension is {y.dims}."
            )
    ...
    if keep_coords in ["keep", "drop"]:
        for key in y.data_vars:
            ds[key] = (
                func(y[key].variable) if any(d in y[key].dims for d in dims) else y[key]
            )
        for key in y.coords:
